...
db_path = os.path.join('C:\\Users\\HP\\bin-main', db_folder, 'mydb.db')
...
def selection_par_roulette(population, fitness_values):
    ...
    probabilites = [fitness / sum(fitness_values) for fitness in fitness_values]

    # Sélectionner un indice d'individu en fonction des probabilités calculées
    indice_selectionne = np.random.choice(len(population), p=probabilites)

    # Renvoyer l'individu sélectionné
...
def croisement(parent1, parent2):
    ...
    point_crossover = random.randint(1, max(len(parent1), len(parent2)))
    # Sélectionner les parties des parents avant et après le point de croisement
    part1_parent1 = parent1[:point_crossover]
    part2_parent2 = parent2[point_crossover:]
    # Enfant résultant du croisement
    enfant = part1_parent1 + [objet for objet in part2_parent2]
    return enfant
...
def algorithme_genetique(objets, capacite_bin, taille_population, taux_mutation, nombre_generations):
    ...
    end_time = time.time()
    timeG= end_time-start_time
    nombre_boites_utilisees = fitness(meilleure_solution, capacite_bin)
    
    return meilleure_solution, nombre_boites_utilisees,timeG
...
#for bin_index, bin_items in enumerate(best_solution):
    ...
    end_time = time.time()
    timeF = end_time-start_time+0.01
    return best_solution,best_solution_bins,timeF
...
def home():
    ...
    subprocess.Popen(["python", "python.py"], cwd=r"C:\Users\HP\bin-main\app\SQLite")
...
@app.route('/afficher_graphe.html')
def afficher_graphe():
    # Ajoutez ici le code pour préparer les données nécessaires à la page index.html
    return render_template('afficher_graphe.html') 
@app.route('/afficher_graphe1.html')
def afficher_graphe1():
    # Ajoutez ici le code pour préparer les données nécessaires à la page index.html
    return render_template('afficher_graphe1.html')
...
@app.route('/ouvrir-base-de-donnees1')
def ouvrir_base_route1():
    ...
    if conn:
        try:
            ...
            return render_template('afficher_benchmarks_pour_graphe.html',  benchmarks_perso=data_benchmarks_perso ,parametre1=parametre1 )
        except Exception as e:
            return f"Erreur lors de la récupération des données : {e}"
    else:
        return "Erreur lors de l'ouverture de la base de données."
@app.route('/ouvrir-base-de-donnees2')
def ouvrir_base_route2():
    parametre1 = request.args.get('parametre1')
    conn = ouvrir_base_de_donnees()  # Appel de la fonction qui ouvre la base de données
    if conn:
        try:
            ...
            return render_template('afficher_benchmarks_pour_graphe1.html',  benchmarks_perso=data_benchmarks_perso ,parametre1=parametre1 )
        except Exception as e:
            return f"Erreur lors de la récupération des données : {e}"
    else:
        return "Erreur lors de l'ouverture de la base de données."
...
def run_algorithm(algorithme):
    ...
    try:
      ...
      if algorithme == 'genetique':
        ...
        meilleure_solution_genetique, nombre_boites_utilisees_genetique,time1 = algorithme_genetique(dimensions_objets, capacite_bin, parametres['taille_population'], parametres['taux_mutation'], parametres['nbr_generations'])
        print("Résultats de l'algorithme :", meilleure_solution_genetique, nombre_boites_utilisees_genetique)
        try:
          return jsonify({"solution": meilleure_solution_genetique, "nombre_boites_utilisees": nombre_boites_utilisees_genetique ,"time":time1})
        except Exception as e:
        ...
        meilleure_solution_fourmis, nombre_boites_utilisees_fourmis,time1 = Algo_fourmis(dimensions_objets, capacite_bin, parametres['nombre_Fourmis'])
        print("Résultats de l'algorithme :", meilleure_solution_fourmis, nombre_boites_utilisees_fourmis)
        try:
          return jsonify({"solution": meilleure_solution_fourmis, "nombre_boites_utilisees": nombre_boites_utilisees_fourmis ,"time":time1})
        except Exception as e:
          print("Erreur lors de la conversion en JSON : {e}")
          return jsonify({"erreur": "Erreur lors de la conversion en JSON"}), 500
...
@app.route('/generer_graphe1', methods=['GET', 'POST'])
def generer_graphe1():
    data = request.get_json()
    selected_benchmarks = data['selected_benchmarks']
    ...
    for benchmark_id in selected_benchmarks:
        # Récupérer les objets du benchmark à partir de la base de données
        # Supposons que vous ayez une fonction pour récupérer les objets d'un benchmark en fonction de son ID
        objets = ast.literal_eval(benchmark_id)
        _, bins_genetic,_ = algorithme_genetique(objets, bin_capacity, taille_population, taux_mutation, nombre_generations)
        _, bins_ant_colony,_ = Algo_fourmis(objets, bin_capacity, nombre_fourmis)
        bins_used_genetic.append(bins_genetic)
        bins_used_ant_colony.append(bins_ant_colony)

    objets_range = range(1, len(selected_benchmarks)+1)
    print(objets_range)
...
@app.route('/generer_graphe2', methods=['GET', 'POST'])
def generer_graphe2():
    data = request.get_json()
    selected_benchmarks = data['selected_benchmarks']
    taille_population = 10
    ...
    for benchmark_id in selected_benchmarks:
        ...
        # Supposons que vous ayez une fonction pour récupérer les objets d'un benchmark en fonction de son ID
        objets = ast.literal_eval(benchmark_id)
        _,_,time1 = algorithme_genetique(objets, bin_capacity, taille_population, taux_mutation, nombre_generations)
        _,_,time2 = Algo_fourmis(objets, bin_capacity, nombre_fourmis)
        timeG.append(time1)
        timeF.append(time2)

    objets_range = range(1, len(selected_benchmarks)+1)
    print(objets_range)
    plt.plot(objets_range, timeG, label='Algorithme génétique')
    plt.plot(objets_range, timeF, label='Algorithme des fourmis', linestyle='--')
    plt.xlabel('Nombre d\'objets')
    plt.ylabel('Temps de convergence')
    plt.title('temps de convergence des deux algorithmes')
    plt.legend()
    plt.savefig('static/graph.png')  # Sauvegarde du graphe en tant qu'image PNG
    plt.close()  
...
@app.route('/graph_image2')
def graph_image1():
    return send_from_directory('static', 'graph1.png')  # Renvoie l'image sauvegardée@app.route('/enregistrer-donnees', methods=['POST'])
@app.route('/generer_graphe')
def generer_graphe():
    objets_range = range(1, 50)
    taille_population = 10
    taux_mutation = 0.1
